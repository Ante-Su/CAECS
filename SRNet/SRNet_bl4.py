    ...
    def _build_model(self, inputs):
        ...
        with arg_scope([layers.conv2d], num_outputs=16,
            ...
            with tf.variable_scope('ensemble'):
                ...
                conv4=layers.conv2d(res, num_outputs=512)
                actv4=tf.nn.elu(layers.batch_norm(conv4))
                ...
                avgp4 = tf.reduce_mean(actv4, reduction_axis,  keep_dims=True )
            with tf.variable_scope('combine'):
                ip1=layers.fully_connected(layers.flatten(avgp1), num_outputs=2,
                ...
                ip2=layers.fully_connected(layers.flatten(avgp2), num_outputs=2,
                    activation_fn=None, normalizer_fn=None,
                    weights_initializer=tf.random_normal_initializer(mean=0., stddev=0.01), 
                    biases_initializer=tf.constant_initializer(0.), scope='ip2')  
                ip3=layers.fully_connected(layers.flatten(avgp3), num_outputs=2,
                    activation_fn=None, normalizer_fn=None,
                    weights_initializer=tf.random_normal_initializer(mean=0., stddev=0.01), 
                    biases_initializer=tf.constant_initializer(0.), scope='ip3')
                ip4=layers.fully_connected(layers.flatten(avgp3), num_outputs=2,
                    activation_fn=None, normalizer_fn=None,
                    weights_initializer=tf.random_normal_initializer(mean=0., stddev=0.01),
                    biases_initializer=tf.constant_initializer(0.), scope='ip4')
                combine = tf.concat([ip1,ip2,ip3,ip4],1)
        ip=layers.fully_connected(combine, num_outputs=2,
                    activation_fn=None, normalizer_fn=None,
                    weights_initializer=tf.random_normal_initializer(mean=0., stddev=0.01), 
